...
</div>`;let gt="general";const me=[{name:"general",markup:xo,button_id:"settings_button_general",function:No},{name:"user",markup:Do,button_id:"settings_button_user",function:jo},{name:"admin",markup:Bo,button_id:"settings_button_admin",function:Uo}];function Ao(){const e=document.getElementById("MAINcontentPages");let t=Ro;for(const[i,s]of Object.entries(C)){const a=new RegExp(`\\{${i}\\}`,"g");t=t.replace(a,s)}e.innerHTML=t;const n=document.getElementById("settings_go_to_other_content_button");qe(gt);for(var o=me.length-1;o>=0;o--){const i=me[o].button_id;document.getElementById(i).addEventListener("click",function(a){const r=a.srcElement.id,d=me.find(m=>m.button_id===r).name;qe(d)})}n.addEventListener("click",function(){J("/")})}function _t(){const e=document.getElementById("MAINcontentPages");e.innerHTML=""}function qe(e){gt=e;const t=["users_img"],n=document.getElementById("view_container"),o=me.find(s=>s.name===e);let i=o.markup;for(const[s,a]of Object.entries(C)){const r=new RegExp(`\\{${s}\\}`,"g");i=i.replace(r,a)}for(let s=0;s<t.length;s++){const a=t[s].toString(),r=new RegExp(`\\{${a}\\}`,"g");let c="";a==="users_img"&&(c=`${I}/${ve}/4/`),i=i.replace(r,c)}n.innerHTML=i,o.function()}function No(){}function jo(){pt("user-id","settings_views_user_pfp_img","update_user_pfp")}let X=[];async function Uo(){if(await xe("admin")!=="true")return;const t=["users_img"],n=document.getElementById("view_container");let o=So;for(const[s,a]of Object.entries(C)){const r=new RegExp(`\\{${s}\\}`,"g");o=o.replace(r,a)}for(let s=0;s<t.length;s++){const a=t[s].toString(),r=new RegExp(`\\{${a}\\}`,"g");let c="";a==="users_img"&&(c=`${I}/${ve}/4/`),o=o.replace(r,c)}n.innerHTML=o,ft(),document.getElementById("SETTINGS_ADMIN_UPDATE_COLUMN_submit_button").addEventListener("click",function(s){const a=document.getElementById("SETTINGS_ADMIN_UDPATE_COLUMN_column_to_updated"),r=document.getElementById("SETTINGS_ADMIN_UDPATE_COLUMN_new_data");if(a.value==""||r.value=="")k("Error Updating Column: empty inputs","error");else for(var c=X.length-1;c>=0;c--){const d=X[c];Qo(d,a.value,r.value)}})}let G;async function ft(){const e=await Fo();G=JSON.parse(e);const t=document.getElementById("usersTableInforRowsContainer");t.innerHTML="";for(let n=0;n<G.length;n++){const o=["Username","Email","UUID","Verified","Space_Used","Storage_Allowance","profile_picture_url","checkbox_number","Admin"],i=G[n].username,s=G[n].uuid,a=G[n].verified,r=G[n].email;let c=G[n].storage_allowance;c=Z(c);const d=G[n].admin,m=n;let p=G[n].storage_used;p=Z(p);let E=Oo,y=G[n].profile_picture;y=`${I}/${y}/1/`;for(let u=0;u<o.length;u++){const g=o[u].toString(),b=new RegExp(`\\{${g}\\}`,"g");let v="";g==="Username"?v=i:g==="Email"?v=r:g==="UUID"?v=s:g==="Verified"?v=a:g==="Space_Used"?v=p:g==="Storage_Allowance"?v=c:g==="profile_picture_url"?v=y:g==="checkbox_number"?v=m:g==="Admin"&&(v=d),E=E.replace(b,v)}t.innerHTML+=E}t.addEventListener("change",function(n){n.target&&n.target.type==="checkbox"&&ko()})}function ko(){X=[];const t=usersTableEnvironment.querySelectorAll('input[type="checkbox"]'),n=[],o=document.getElementById("settings_admin_selected_items_visualition");t.forEach(function(i){if(i.checked){const r={id:i.id.replace("admin_users_table_check_box_",""),value:i.value};n.push(r)}}),n.forEach(function(i){const a=G[i.id].uuid;X.push(a)}),o.innerHTML=`Selected Items: ${X.length}`}async function Go(){const e=await xe("last_state"),t=JSON.parse(e),n=new Map;setInterval(s,1e4);let o=i(t);if(window.addEventListener("popstate",a),t!=null){const r=window.location.pathname;J(r),Fi(t)}else a();function i(r){return JSON.stringify(r)}function s(){const r=window.location.pathname,c=l,d=_,m=Zi();m.current_path=r,m.current_queue=c,m.current_index=d;const p=i(m);p!==o&&($o("last_state",m),o=p,n.set("playback_states",p))}function a(){J(currentPath)}}function Jo(){yt(),_t(),Co()}function qo(e){lt(),_t(),Zo(e)}function He(){lt(),yt(),Ao()}const $e={"/":Jo,"/new-project/":le,"/new-project":le,"/new/":le,"/new":le,"/projects/:projectID":qo,"/settings/":He,"/settings":He};function Ho(){const e=window.location.pathname,t=document.getElementById("MAINcontentPages");setTimeout(n,4e3),t.innerHTML+=`
    <br><br>
    <br><br>
    <br><br>    
