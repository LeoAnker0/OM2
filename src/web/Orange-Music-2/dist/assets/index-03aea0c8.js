...
</div>`;async function dn(e,t){const n=new XMLHttpRequest,o=new FormData,i=localStorage.getItem("JWT");o.append("file",e),o.append("project_id",t),o.append("jwt",i),n.upload.onprogress=function(s){s.lengthComputable&&s.loaded/s.total*100},n.onload=function(){so()},n.onerror=function(){console.log("there was an error with the upload")},n.open("POST",`${h}/files/upload/photo/`,!0),n.send(o)}async function un(e,t){Array.isArray(e)||(e=[e]);for(const n of e)await dn(n,t)}async function le(e,t,n){try{const o=localStorage.getItem("JWT");if(!o){console.log("no jwt");return}const i={"access-token":o,project_id:e,column_to_be_updated:t,new_data:n};if((await(await fetch(`${h}/projects/update_details/`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(i)})).json()).updated==="success")return;console.log("there was an error")}catch(o){console.error("Error:",o)}}async function mn(e,t){try{const n=localStorage.getItem("JWT");if(!n){console.log("no jwt");return}const o={"access-token":n,column_to_be_updated:e,new_data:JSON.stringify(t)};if((await(await fetch(`${h}/users/update_details/`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)})).json()).updated==="success")return;console.log("there was an error")}catch(n){console.error("Error:",n)}}async function Se(e){try{const t=localStorage.getItem("JWT");if(!t){console.log("no jwt");return}const n={"access-token":t,wanted_column:e};return(await(await fetch(`${h}/users/get_details/`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)})).json()).response}catch(t){console.error("Error:",t)}}async function $(e){try{const t=localStorage.getItem("JWT");if(!t){console.log("no jwt");return}const n={"access-token":t,project_id:e};return(await(await fetch(`${h}/projects/get-project-details/`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)})).json()).project_details}catch(t){console.error("Error:",t)}}async function pn(){try{const e=localStorage.getItem("JWT");if(!e)return console.log("no jwt"),[];const t={"access-token":e},i=(await(await fetch(`${h}/projects/get-projects/`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})).json()).projects;return Array.isArray(i)?i.map(a=>({img:a.picture_url,top:a.project_name,bottom:a.project_contributors,days:a.time_created,project_id:a.project_id})):(console.log("Projects is not an array:",i),[])}catch(e){return console.error("Error:",e),[]}}const gn=`<div class="MOG-container" id="MOGcontainer">
    <div class="MOG-grid" id="MOGgridContainer"></div>
</div>`,_n=`<button class="MOG-itemContainer" id="MOGI_itemContainer-{MOGI_placeholder_itemID}" draggable="false">
...
</button>`,fn=`<button id="MOGaddNewItem" class="MOGaddNewItem">
...
</button>`;async function vn(){try{const e=document.getElementById("MAINcontentPages");yn();const t=await pn();Tn(t);return}catch(e){console.error("Error in initMusicObjectsGrid:",e)}}function En(){const e=document.getElementById("MAINcontentPages"),t=document.getElementById("MOGcontainer");t!==null&&t.removeEventListener("click",Re),e.innerHTML=""}function yn(){let e="MAINcontentPages",t=gn;for(const[n,o]of Object.entries(y)){const i=new RegExp(`\\{${n}\\}`,"g");t=t.replace(i,o)}document.getElementById(e).innerHTML+=t}function Tn(e){const t=document.getElementById("MOGgridContainer");t.innerHTML="";let n=fn;for(const[r,l]of Object.entries(y)){const d=new RegExp(`\\{${r}\\}`,"g");n=n.replace(d,l)}t.innerHTML+=n;const o=e.length;for(var i=0;i<=o-1;i++){let r=_n;const l=["MOG_image","MOG_text1","MOG_text2","MOG_checkedDate","MOGI_placeholder_itemID"],d=i,g=e[i].img,_=e[i].top,I=e[i].bottom,O=e[i].days,L=hn(O);for(const[p,v]of Object.entries(y)){const f=new RegExp(`\\{${p}\\}`,"g");r=r.replace(f,v)}for(let p=0;p<l.length;p++){const v=l[p].toString(),f=new RegExp(`\\{${v}\\}`,"g");let C="";v==="MOG_image"?C=`${E}/${g}/4/`:v==="MOG_text1"?C=_:v==="MOG_text2"?C=I:v==="MOG_checkedDate"?C=L:v==="MOGI_placeholder_itemID"&&(C=d),r=r.replace(f,C)}t.innerHTML+=r}const s=document.getElementById("MOGcontainer");s.addEventListener("click",function(r){Re(r,e)});let a;s.addEventListener("touchstart",function(r){r.timeStamp,a=setTimeout(function(){c(r,e)},500)}),s.addEventListener("touchend",function(r){r.timeStamp,clearTimeout(a)});function c(r,l){navigator.vibrate(1e3);const g=r.target.id.split("-")[1],_=l[g].project_id;console.log(_),De(r,_)}}async function bn(e){try{const t=localStorage.getItem("JWT");if(!t){console.log("no jwt");return}const n={"access-token":t,project_id:e};return(await(await fetch(`${h}/projects/get-project-details/`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)})).json()).project_details}catch(t){console.error("Error:",t)}}async function Re(e,t){const n=e.target;if(n.classList.contains("MOG-item-controls-play")){const o=n.id.split("-")[1],i=t[o].project_id,s=await bn(i);qe(s)}if(n.classList.contains("MOG-item-controls-menu")){const o=n.id.split("-")[1],i=t[o].project_id;De(e,i)}if(n.classList.contains("MOG-itemContainer")&&n.id!=="MOGaddNewItem"){const o=n.id.split("-")[1],i=t[o].project_id;G(`/projects/${i}`)}n.id==="MOGaddNewItem"&&In()}function De(e,t){e.stopPropagation(),D([{displayText:"Play next",optionalSVG:"icons_playlist",optionalParams:{PROJECT_ID:t,QUEUE_POSITION:"next"},function:"PLAYBACK_add_songs_to_queue"},{displayText:"Play later",optionalSVG:"icons_playlist",optionalParams:{PROJECT_ID:t,QUEUE_POSITION:"later"},function:"PLAYBACK_add_songs_to_queue"}],e)}function hn(e){const n=(Date.now()-e)/(1e3*60*60*24);let o;return n<.01||n<1?(o="Now",o):n<8?(o=Math.floor(n)+"d",o):n<29?(o=Math.floor(n/7)+"w",o):n<365?(o=Math.floor(n/28)+"m",o):(o=Math.floor(n/365)+"y",o)}function In(){G("/new-project/")}async function Cn(){function e(){const c=window.location.pathname;G(c)}window.addEventListener("popstate",e);const t=await Se("last_state"),n=JSON.parse(t[0].last_state);if(n!=null){const c=window.location.pathname;G(c),Bo(n)}else e();let o="";const i=new Map;function s(c){return JSON.stringify(c)}function a(){const c=window.location.pathname,r=u,l=m,d=xo();d.current_path=c,d.current_queue=r,d.current_index=l;const g=s(d);g!==o&&(mn("last_state",d),o=g,i.set("playback_states",g))}setInterval(a,1e4)}function wn(){Dn(),vn()}function Ln(e){En(),Rn()}const be={"/":wn,"/new-project/":Sn,"/projects/:projectID":Ln};function Pn(){console.log("notFoundHandler")}function G(e){const t=Object.keys(be);for(const n of t){const o=new RegExp(`^${n.replace(/:[^\s/]+/g,"([^/]+)")}$`),i=e.match(o);if(i){const s=be[n],a=i.slice(1);s(...a),e!=="/new-project/"&&history.pushState({},"",e);return}}Pn()}const Bn=`<div class="PROJECTviewContainerEnvironment">
