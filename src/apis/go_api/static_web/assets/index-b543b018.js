...
</div>`,St=`<div class="TOPRIGHT-topHalf-right-content">
    <!-- playback controls -->
...
</div>`,zn=`<button id="MOGaddNewItem" class="MOGaddNewItem">
...
</button>`,Vn=`<button class="MOG-itemContainer" id="MOGI_itemContainer-{MOGI_placeholder_itemID}" draggable="false">
...
<p>Double click your profile picture to change it.</p>`,ao=`<div class="settings_container">
...
</div>`;let ye;async function co(){const e=document.getElementById("TOPRIGHTaccountImg");try{const t=await be("profile_picture"),n=`${w}/${t}/1`;ye=t;const o=document.getElementById("TOPRIGHTaccountImgButton");e.src=n,o.addEventListener("click",ro)}catch(t){console.error("Error initializing account image:",t)}}function ro(e){e.stopPropagation(),lo(e)}function lo(e){J([{displayText:"Help",optionalSVG:"icons_helpIcon",function:"None"},{displayText:"Settings",optionalSVG:"icons_settings",function:"OPEN_SETTINGS_PAGE"},{displayText:"Sign Out",optionalSVG:"None",function:"SIGN_OUT_USER"}],e)}async function uo(){await Co()}let Xe="general";const ce=[{name:"general",markup:io,button_id:"settings_button_general",function:go},{name:"user",markup:so,button_id:"settings_button_user",function:po},{name:"admin",markup:no,button_id:"settings_button_admin",function:_o}];function mo(){const e=document.getElementById("MAINcontentPages");let t=ao;for(const[i,s]of Object.entries(L)){const a=new RegExp(`\\{${i}\\}`,"g");t=t.replace(a,s)}e.innerHTML=t;const n=document.getElementById("settings_go_to_other_content_button");Me(Xe);for(var o=ce.length-1;o>=0;o--){const i=ce[o].button_id;document.getElementById(i).addEventListener("click",function(a){const c=a.srcElement.id,r=ce.find(m=>m.button_id===c).name;Me(r)})}n.addEventListener("click",function(){k("/")})}function Ze(){const e=document.getElementById("MAINcontentPages");e.innerHTML=""}function Me(e){Xe=e;const t=["users_img"],n=document.getElementById("view_container"),o=ce.find(s=>s.name===e);let i=o.markup;for(const[s,a]of Object.entries(L)){const c=new RegExp(`\\{${s}\\}`,"g");i=i.replace(c,a)}for(let s=0;s<t.length;s++){const a=t[s].toString(),c=new RegExp(`\\{${a}\\}`,"g");let l="";a==="users_img"&&(l=`${w}/${ye}/4/`),i=i.replace(c,l)}n.innerHTML=i,o.function()}function go(){}function po(){We("user-id","settings_views_user_pfp_img","update_user_pfp")}async function _o(){if((await be("admin"))[0].admin!==!0)return;const n=["users_img"],o=document.getElementById("view_container");let i=to,s=[];for(const[r,m]of Object.entries(L)){const p=new RegExp(`\\{${r}\\}`,"g");i=i.replace(p,m)}for(let r=0;r<n.length;r++){const m=n[r].toString(),p=new RegExp(`\\{${m}\\}`,"g");let b="";m==="users_img"&&(b=`${w}/${ye}/4/`),i=i.replace(p,b)}o.innerHTML=i;const a=await Io(),c=document.getElementById("usersTableInforRowsContainer");c.innerHTML="";for(let r=0;r<a.length;r++){const m=["Username","Email","UUID","Verified","Space_Used","profile_picture_url","checkbox_number","Admin"],p=a[r].username,b=a[r].uuid,E=a[r].verified,d=a[r].email,_=a[r].admin,g=r;let h=a[r].storage_used;h=Fe(h);let y=eo,O=a[r].profile_picture;O=`${w}/${O}/1/`;for(let P=0;P<m.length;P++){const f=m[P].toString(),I=new RegExp(`\\{${f}\\}`,"g");let C="";f==="Username"?C=p:f==="Email"?C=d:f==="UUID"?C=b:f==="Verified"?C=E:f==="Space_Used"?C=h:f==="profile_picture_url"?C=O:f==="checkbox_number"?C=g:f==="Admin"&&(C=_),y=y.replace(I,C)}c.innerHTML+=y}c.addEventListener("change",function(r){r.target&&r.target.type==="checkbox"&&l()});function l(){s=[];const m=c.querySelectorAll('input[type="checkbox"]'),p=[],b=document.getElementById("settings_admin_selected_items_visualition");m.forEach(function(E){if(E.checked){const g={id:E.id.replace("admin_users_table_check_box_",""),value:E.value};p.push(g)}}),p.forEach(function(E){const _=a[E.id].uuid;s.push(_)}),b.innerHTML=`Selected Items: ${s.length}`}}async function fo(){const e=await be("last_state"),t=JSON.parse(e),n=new Map;setInterval(s,1e4);let o="";if(window.addEventListener("popstate",a),t!=null){const c=window.location.pathname;k(c),Ei(t)}else a();function i(c){return JSON.stringify(c)}function s(){const c=window.location.pathname,l=u,r=v,m=Ci();m.current_path=c,m.current_queue=l,m.current_index=r;const p=i(m);p!==o&&(ho("last_state",m),o=p,n.set("playback_states",p))}function a(){k(currentPath)}}function vo(){ot(),Ze(),Kn()}function Eo(e){ze(),Ze(),Bo(e)}function yo(){ze(),ot(),mo()}const Oe={"/":vo,"/new-project/":To,"/projects/:projectID":Eo,"/settings/":yo};function bo(){const e=window.location.pathname,t=document.getElementById("MAINcontentPages");setTimeout(n,4e3),t.innerHTML+=`
    <br><br>
    <br><br>
    <br><br>    
    ...
    `;function n(){k("/")}}function k(e){const t=Object.keys(Oe);for(const n of t){const o=new RegExp(`^${n.replace(/:[^\s/]+/g,"([^/]+)")}$`),i=e.match(o);if(i){const s=Oe[n],a=i.slice(1);s(...a),e!=="/new-project/"&&history.pushState({},"",e);return}}bo()}async function ho(e,t){try{const n={column_to_be_updated:e,new_data:JSON.stringify(t)};if((await(await fetch(`${S}/users/update_user_details`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify(n)})).json()).updated==="success")return;console.log("there was an error")}catch(n){console.error("Error:",n)}}async function be(e){try{return(await(await fetch(`${S}/users/get_user_details/${e}`,{method:"GET",headers:{"Content-Type":"application/json"},credentials:"include"})).json()).response}catch(t){console.error("Error:",t)}}async function W(e,t,n){try{const o={project_id:e,column_to_be_updated:t,new_data:n};if((await(await fetch(`${S}/projects/update_details`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify(o)})).json()).updated==="success")return"yes";console.log("there was an error, yes")}catch(o){console.error("Error:",o)}}async function le(e){try{return(await(await fetch(`${S}/projects/get_project_details/${e}`,{method:"GET",headers:{"Content-Type":"application/json"},credentials:"include"})).json()).project_details}catch(t){console.error("Error:",t)}}async function et(e,t){try{const i=(await(await fetch(`${S}/projects/get_projects?l=${e}&c=${t}`,{method:"GET",headers:{"Content-Type":"application/json"},credentials:"include"})).json()).projects;return Array.isArray(i)?i.map(a=>({img:a.PictureURL,top:a.ProjectName,bottom:a.ProjectContributors,days:a.TimeCreated,project_id:a.ProjectID})):[]}catch(n){return console.error("Error:",n),[]}}async function To(){try{const o=`/projects/${(await(await fetch(`${S}/projects/get_new_project_id`,{method:"GET",headers:{"Content-Type":"application/json"},credentials:"include"})).json()).projectID}`;k(o)}catch(e){console.error("Error:",e)}}async function Io(){try{return(await(await fetch(`${S}/admin/get_users_table/`,{method:"GET",headers:{"Content-Type":"application/json"},credentials:"include"})).json()).response}catch(e){console.error("Error:",e)}}async function Co(){try{const t=await(await fetch(`${S}/meta/signout`,{method:"GET",headers:{"Content-Type":"application/json"},credentials:"include"})).json();console.log(t)}catch(e){console.error("Error:",e)}}const Se=1500;let tt=0,fe=[],pe;function F(e,t){const o=new Date().getTime();fe.push({notificationText:e,notificationType:t,time:o}),nt()}function nt(){const t=new Date().getTime(),n=fe.at(tt);fe.length>1?t-pe>Se?(xe(n.notificationText,n.notificationType),pe=t):setTimeout(nt,Se):(xe(n.notificationText,n.notificationType),pe=t)}function xe(e,t){const n=document.getElementById("noticationContainer");let o;tt+=1,t=="error"?o="var(--whoopsie)":t=="success"?o="var(--success)":t=="background"?o="var(--background)":t=="orange"?o="var(--orange)":t=="primary"?o="var(--primary)":t=="secondary"&&(o="var(--secondary)");const i=document.createElement("div");i.style.backgroundColor=o,i.textContent=e,n.appendChild(i)}const wo=`<div class="PROJECTview-projectTable" id="PROJECTview-projectTable" draggable="false" style="user-select: none; pointer-events: none;">
...
-->`;const S="/apis",w="/media";async function Lt(){hi(),Mn(),On(),di(),Oi(),ji(),Ni(),co(),Zo(),fo()}if(!window.hasCodeRun){let e=Ki;for(const[t,n]of Object.entries(L)){const o=new RegExp(`\\{${t}\\}`,"g");e=e.replace(o,n)}document.getElementById("app").innerHTML=e,Yi(),window.hasCodeRun=!0}
